...
const fetchBalance = async (donation: Donation): Promise<Donation> => {
  try {
    ...
    const networkMapping: { [key: string]: { type: string, chainId?: string } } = {
    ...
    if (networkConfig && (networkConfig.type === "evm" || networkConfig.type === "svn")) {
      try {
        // Use Pinax Token API for EVM and Solana chains
        const apiEndpoint = networkConfig.type === "evm" 
        ...
        if (response.ok) {
          ...
          if (data && data.balances) {
            const nativeToken = data.balances.find((b: any) => 
              b.symbol === donation.network || b.isNative === true
            );
            if (nativeToken) {
